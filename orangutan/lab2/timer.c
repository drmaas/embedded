...
void init_timers() {
        ...
        // -------------------------  TIMER3 setup for PID --------------------------------------//
        // Software Clock Using Timer/Counter 3.
        // THE ISR for this is below.

        // SET appropriate bits in TCCR for CTC mode. This is mode 4.
        TCCR3A &= ~(1 << WGM30);
        TCCR3A &= ~(1 << WGM31);
        TCCR3B |= (1 << WGM32);
        TCCR3B &= ~(1 << WGM33);
        ...
        // Using pre-scaler 256, using CS3(2:0) = 4
        TCCR3B |= (1 << CS32);
        TCCR3B &= ~(1 << CS31);
        TCCR3B &= ~(1 << CS30);

        //Enable output compare match interrupt on timer 3
        TIMSK3 |= ( 1 << OCIE3A );

        //--------------------------- TIMER0 setup for interpolater ----------------------------------//
        // SET appropriate bits in TCCR ...
        // Using CTC mode with OCR0A for TOP. This is mode 2, thus WGM0/210 = 0010.
        TCCR0A &= ~(1 << WGM00);
        TCCR0A |= (1 << WGM01);
        TCCR0B &= ~(1 << WGM02);
        ...
        // Interrupt Frequency: 500 = f_IO / (prescaler*OCR0A)
        prescalar = 256;
        frequency = 500.00;
...
//TIMER3 Interrupt for PID at 1KHz
...
//TIMER0 Interrupt for Interpolater at 500Hz
//PID equation is T = Kp(Pr - Pm) - Kd*Vm
